    ...
    // call respective methods.
   	...
   	if(objAjax)
   	{
	    ...
	    htmlErrors.addError("confirmInfo","Do you want to delete Sales Order Detail(s)?",  false);
   		...
   		else if(docview==6304)
   		{
   			//Tracker#:26957 FR02 CREATE NEW SCREEN - CUSTOMER INVOICE
   			htmlErrors.addError("confirmInfo","Do you want to delete Customer Invoice?",  false);
		}
   		else if(docview==6305)
   		{
   			if(objHTMLData!=null && objHTMLData.hasUserModifiedData()==false)
		   	{
		   		var htmlErrors = objAjax.error();
   			...
   			htmlErrors.addError("confirmInfo","Do you want to delete Customer Invoice Detail(s)?",  false);
   		}
...
function deleteColor()
{
    ...
    //alert("sectionName  " + sectionName);
	    ...
	    if(objAjax && objHTMLData)
	    {
	       ...
	       {
	    	   url = "delete";	        	
	    	   Main.loadWorkArea("customerinvoiceoverview.do", url,'','refreshNavigationGridOnDelete');
	    	   return;
	       }
	       if(docview==6305)
	       {
...
function uploadDone()
{
	...
	{
		tabSelectionAction(null, 'designintent.do', 'db_refresh', null, null);
	}
	else if (docview==186)
	{
    	if (selectedTab != 'undefined' && 
    			typeof $("#_tab0CenterCell") != 'undefined'
    			&& $("#_tab0CenterCell").length > 0)
	    {	
    		var slctdTab = '#'+selectedTab;
    		$(slctdTab).click();
    	}
	}
