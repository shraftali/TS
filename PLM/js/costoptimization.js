...
var CO = {};
//Tracker#:26621 FR10 PROVIDE THE ABILITY TO DELETE ITEM CHANNEL(S) FROM COST OPTIMIZATION SCREEN
var selectMsg="Please select one or more rows.";
var addChannelMsg = "This process can only be triggered at the Quote level.  Please select one or more Quotes to trigger Add Channels.";
var offerSelectMsg="This process can only be executed at the Quote Channel level.  Please select one or more Quote Channels to delete.";
...
//Tracker#:26621 FR10 PROVIDE THE ABILITY TO DELETE ITEM CHANNEL(S) FROM COST OPTIMIZATION SCREEN
 CO.deleteItemChannels = function deleteItemChannels()
 {    
	...
	{		 
		...
		{			 
		     ...
		     htmlErrors.addError("confirmInfo", szMsg_Changes,  false);
		     var saveFunc = "\"CO.save()\"";
		     var canceFunc = "\"CO.verifyPriceExceptionSelected(\"delete\","+offerSelectMsg+")\"";
		     messagingDiv(htmlErrors, saveFunc, canceFunc);
		 }
		 ...
		 {
			 objAjax = new htmlAjax();
			 objAjax.error().addError("warningInfo", offerSelectMsg, false);
			 _displayProcessMessage(objAjax);
			 objAjax=null;
		 }
	 }
     ...
     {	
         CO.verifyPriceExceptionSelected("delete",offerSelectMsg);	   
	 }
  }
...
//This method generates the pop up to prompt the user to delete the channels or cancel the deletion
CO.showDeletePopUp = function showDeletePopUp()
{
  var htmlAreaObj = _getWorkAreaDefaultObj();
  var objAjax = htmlAreaObj.getHTMLAjax();
  var htmlErrors = objAjax.error();
  var confirmMsg="Are you sure you want to delete the selected Channel(s)?";
  htmlErrors.addError("confirmInfo", confirmMsg,  false);
  var saveFunc="\"CO.deleteChannelsProcess()\"";
  var canceFunc="closeMsgBox()";
  messagingDiv(htmlErrors,saveFunc, canceFunc);
}

//This method handles the navigation flow to the CostOptimization Delete Channels process
CO.deleteChannelsProcess = function deleteChannelsProcess()	 
{
  bShowMsg=true;
  objAjax = new htmlAjax();
  objAjax.setActionURL("costoptimization.do");
  objAjax.setActionMethod("deletechannels");
  objAjax.setProcessHandler(CO.pResp);
  objAjax.parameter().add("chgflds", objHtmlData.getChangeFields());
  objAjax.sendRequest();
  closeMsgBox();	 
...
//This method checks for the Quote.Channel level record and calls addItemChannelProcess if any record is selected 
CO.addItemChannels = function addItemChannels()
{     
	
	var objHtmlData  = _getAreaHTMLDataObjByDocView(16501);
	var objAjax = new htmlAjax();
	bShowMsg = true;
	if(objHtmlData!=null && (objHtmlData.getChangeFields()!=null && objHtmlData.getChangeFields()!=""))
	{	
		...
		if(objHtmlData.hasUserModifiedData() && objHtmlData.isDataModified())
		{
			...
			htmlErrors.addError("confirmInfo", szMsg_Changes,  false);
			var saveFunc = "\"CO.save()\"";
			var canceFunc = "\"CO.addItemChannelsProcess()\"";
			messagingDiv(htmlErrors, saveFunc, canceFunc);
		}

		else
		{ 
			CO.addItemChannelsProcess();
		}
	...
	{
		CO.verifyPriceExceptionSelected("add",addChannelMsg);
	}   
}

CO.verifyPriceExceptionSelected = function verifyPriceExceptionSelected(action,message)
{
	
	var objHtmlData  = _getAreaHTMLDataObjByDocView(16401);
	var objAjax = new htmlAjax();
	...
	if(objHtmlData!=null && (objHtmlData.getChangeFields()!=null && objHtmlData.getChangeFields()!=""))
	{
		...
		if(objHtmlData.hasUserModifiedData() && objHtmlData.isDataModified())
		{
		    ...
		    htmlErrors.addError("confirmInfo", szMsg_Changes,  false);
		    var saveFunc = "\"CO.save()\"";
		    var canceFunc = "\"CO.verifyOfferChannelsSelected('"+action+"','"+message+"')\"";
		    messagingDiv(htmlErrors, saveFunc, canceFunc); 
	    }
	    ...
	    {
			objAjax = new htmlAjax();
			objAjax.error().addError("warningInfo", message, false);
			_displayProcessMessage(objAjax);
			objAjax=null; 
		 }
	 ...
	 {
         CO.verifyOfferChannelsSelected(action,message);
     }			
}

//This method handles the checking process of whether the selected record is quote channel record or offer channel record	 
CO.verifyOfferChannelsSelected = function verifyOfferChannelsSelected(action,message)
{
	
	var objHtmlData  = _getAreaHTMLDataObjByDocView(16502);
	var objAjax = new htmlAjax();
	...
	{	
		...
		if(objHtmlData.hasUserModifiedData() && objHtmlData.isDataModified())
		{
			...
			htmlErrors.addError("confirmInfo", szMsg_Changes,  false);
			var saveFunc = "\"CO.save()\"";
			var canceFunc = "\"CO.verifyOfferSelected('"+action+"','"+message+"')\"";
			messagingDiv(htmlErrors, saveFunc, canceFunc);
		}
	...
	{
		CO.verifyOfferSelected(action,message);
	}   
}


//This method handles the checking process of whether the selected record is quote channel record or offer channel record	 
CO.verifyOfferSelected = function verifyOfferSelected(action,message)
{
	...
	{	
		...
		if(objHtmlData.hasUserModifiedData() && objHtmlData.isDataModified())
		{
			...
			htmlErrors.addError("confirmInfo", szMsg_Changes,  false);
			var saveFunc = "\"CO.save()\"";
			var canceFunc = "\"CO.verifyQuoteRowSelected('"+action+"','"+message+"')\"";
			messagingDiv(htmlErrors, saveFunc, canceFunc);
		}
	...
	{
		CO.verifyQuoteRowSelected(action,message);
	}   
}
...
//This method handles the checking process of whether the selected record is quote channel record or quote record
CO.verifyQuoteRowSelected= function verifyQuoteRowSelected(action,message)
{
	var objHtmlData  = _getAreaHTMLDataObjByDocView(140);	 
	var objAjax = new htmlAjax();
	...
	{		
		if(objHtmlData.hasUserModifiedData() && objHtmlData.isDataModified())
		{
			...
			var canceFunc = null;
			if(action=="add"){
				canceFunc = "\"CO.addItemChannelsProcess('"+action+"','"+message+"')\"";
			}else{
				
				canceFunc = "\"CO.showDeletePopUp()\"";
			}
			messagingDiv(htmlErrors, saveFunc, canceFunc); 
		}
		else
		...
		//If no record is checked display the select rows message
		//else display other than quote channel selection message
		...
		if(chkboxes)
		{
			...
			{			
				...
				objAjax = new htmlAjax();
				objAjax.error().addError("warningInfo", selectMsg, false);
				_displayProcessMessage(objAjax);
				objAjax=null;  
			}
		}
		...
		//Variable to denote quote channel section
		var objHtmlData  = _getAreaHTMLDataObjByDocView(16501); 
		...
		{	
			...
			if(objHtmlData.hasUserModifiedData() && objHtmlData.isDataModified())
			{
				...
				htmlErrors.addError("confirmInfo", szMsg_Changes,  false);
				var saveFunc = "\"CO.save()\"";
				var canceFunc = "\"CO.showDeletePopUp()\"";
				messagingDiv(htmlErrors, saveFunc, canceFunc); 
			}
...
//This method generates the item channel list by calling the CostOptimization.java 
CO.addItemChannelsProcess = function addItemChannelProcess()	 
{
	
	objAjax = new htmlAjax();
	objAjax.setActionURL("costoptimization.do");
	objAjax.setActionMethod("additemchannels");
	objAjax.setProcessHandler(CO.pResp);
	objAjax.parameter().add("chgflds", objHtmlData.getChangeFields());
	objAjax.sendRequest();
	closeMsgBox();	 
