...
{	
	if(arguments.length > 0)
	{
		...
		var refreshURL = getElemnt("fileEditRefreshURL");
		if(refreshURL)
		{
			if (refreshURL.value=='refreshSelectedTab')
			{
				document.getElementById(selectedTab).click();
			}
			else
			{	
				loadWorkArea(refreshURL.value, "view");
			}
		...
		else if (typeof(refreshWorkArea) != 'undefined')
        {
        	refreshWorkArea();
			var errorObj = new htmlErrors();
			if (uploaded)
...
function buildUploadURL(componentinfo)
{
	...
	var randString=getRandomString(30);
 	
	pathName = pathName.replace("/", "");
	pathName = pathName.substring(0, pathName.indexOf("/") + 1);  // context path
	if (IE)
	{
		return protocol + "//" + host + "/" + pathName + "upload.do?act=single&editreplace=Y&sessionkey=" + randString + "&componentinfo=" + componentinfo;
	}
	else
	{
		return protocol + "//" + host + "/" + pathName + "upload.do?act=single&editreplace=Y&sessionkey=" + randString + "&componentinfo=" + componentinfo + "&client=java";
	}
}
...
function getRandomString(num)
{
	...
	for (i=0; i <= num; i++)
	{
		uid += Math.round((Math.random()*10)).toString();
	}
	return uid;
}
