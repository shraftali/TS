...
TRADESTONE.Calendar = function (_id, _docViewId) {
    ...
    this.show = function() {
        ...
        //Add refresh button
        $('div.fc-button-today').parent('td').after('<td><a onclick="cal_displayEvents(&#39;' + docViewId + '&#39;)">' + buildButton("Refresh", false) + "</a></td>");
        ...
        var datePickerHTML = '<td style="padding-left:10px">' +
                             ...
                             '</div>' +
                             '</td>';
        $('h2.fc-header-title').parent('td').after(datePickerHTML);
        datePicker = $("#datePicker").datepicker({
            ...
            onSelect: function(dateText, inst) {
                ...
                fullCalendar.fullCalendar('changeView', 'basicDay').fullCalendar('gotoDate', new Date(d));
            }
        });
    ...
    this.moveToDay = function(_date) {
        fullCalendar.fullCalendar('changeView', 'basicDay').fullCalendar('gotoDate', new Date(_date));
    }
    ...
    function setupCalendar() {
        fullCalendar = $(id).fullCalendar({
            ...
            eventSources:[
                    eventSourceFunc = function (start, end, callback) {
                        if (isNotNull(eventSourceStatic)) {
                            fullCalendar.fullCalendar('removeEventSource', eventSourceStatic);
                        }
            ...
            viewDisplay:function (view) {
                ...
                //Handle custom button on/off opacity (see if we can do better)
                var views = ['month', 'basicWeek', 'basicDay'];
                var classes = ['div.fc-button-month', 'div.fc-button-basicWeek', 'div.fc-button-basicDay'];

                for (var i = 0; i < classes.length; ++i) {
                    var objTable = $(classes[i]).find("table");
                    objTable.removeClass();

                    if (view.name == views[i]) {
                        objTable.addClass("fc-tss-button-faded");
                    } else {
                        objTable.addClass("fc-tss-button");
                    }
                }

                toggleTodayButton(view);
            }
        });
    }
    ...
    function customEventRender(event, element, view) {
        ...
        if (objEvent && objElement) {
            ...
            //Place floated right element first (IE6)
            if (!($(objElement).hasClass("fc-corner-right"))) {
                $(objElement).find('span.fc-event-title').before('<img src="' + _ICON_ARR_RIGHT + '" style="float:right;vertical-align:middle;"/>');
            }
            if (!$(objElement).hasClass("fc-corner-left")) {
                $(objElement).find('span.fc-event-title').before('<img src="' + _ICON_ARR_LEFT + '" style="vertical-align:middle;"/>');
            }
            ...
            if (!ismore) {
                ...
                if (!$(objElement).data('qtip')) {
                    $(objElement).qtip({
                        content : getTipContent(objEvent),
    ...
    function setBuyPlanRiskIndicator(objEvent, objElement, isDayView) {
        ...
        $(objElement).find('span.fc-event-title').before('<span class="fc-buyplan-indic">' + content + '</span>');
    }
    ...
    function formatTipDate(date) {
        return $.fullCalendar.formatDate(date, "ddd MMM dd yyyy");
    }
