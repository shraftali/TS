 ...
 *
 */
//TSS added more.. - base code by Serg
(function($, undefined) {
	...
	this.each(function() {
		...
		function fetchEventSource(src, callback) {
			...
			if (typeof src == 'string') {
                ...
                params[options.startParam] = eventStart.getTime();
                //params[options.endParam] = Math.round(eventEnd.getTime() / 1000);
                params[options.endParam] = eventEnd.getTime();
...
function Grid(element, options, methods, viewName) {
    ...
    //TSS change
    element.addClass('fc-grid').css('overflow', 'visible');

    function generateMoreBoxByDate(date)
    {
      var str =
             '<div class="ui-box-container ui-box-outline ui-draggable" id="uibox_'+formatDateForId(date)+'" style="display: none;">';
      str +=    '<div><div class="ui-box-title ui-box-text">';
                   str += formatDateForTitle(date);
      str +=      '</div><div id="uibox_close_'+formatDateForId(date)+'" class="ui-box-close"><a><img class="ui-box-image" src="images/close_top.gif"/> </a></div>';
      str +=    '</div>';
      str +=    '<div id="uibox_content_'+formatDateForId(date)+'" class="ui-box-content"></div>';
      str += '</div>';
      return str;
    }
	...
	function renderGrid(r, c, colFormat, showNumbers) {
		...
		if (!tbody) { // first time, build all cells from scratch
            ...
            var prevMonth; //TSS change
            for (i=0; i<rowCnt; i++) {
				s += "<tr class='fc-week" + i + "'>";
				for (j=0; j<colCnt; j++) {
					//--TSS added id--
                    s += "<td id='" + formatDateForId(d) + "' class='fc-" +
						dayIDs[d.getDay()] + ' ' + // needs to be first
						...
						' fc-not-today') + "'>" +
						 //--TSS provide hyperlink to day, added class fc-day-number-header, formatted day number--
                         (showNumbers ? "<div class='fc-day-number" + (+d == +today ? " fc-day-number-header-today" : " fc-day-number-header") + "'><a href='#' onClick='cal_moveToDay(\""+d+"\");return false'>"+ (((i==0 && j==0) || (d.getMonth() != prevMonth)) ? $.fullCalendar.formatDate(d, "MMMM d") : d.getDate())+"</a></div>" : '') +
                         //(showNumbers ? "<div class='fc-day-number" + (+d == +today ? " fc-day-number-header-today" : " fc-day-number-header") + "'><a href='#' onClick='cal_moveToDay(\""+d+"\");return false'>"+ d.getDate()+"</a></div>" : '') +
                        "<div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div>" +
                         generateMoreBoxByDate(d) +
                         "</td>";
                    prevMonth = d.getMonth();//TSS change
                    addDays(d, 1);
					if (nwe) {
						skipWeekend(d);
					}
			...
			clearEvents();
            if (view.name == 'month')//TSS change
                $("div.ui-box-container").remove();
			...
			d = cloneDate(view.visStart);
            var index = 0;//TSS change
            var prevMonth;
            tbody.find('td').each(function() {
				var td = $(this);
                //TSS - reset id on view change
                td.attr('id', d.getFullYear() + "-" + d.getMonth() + "-"  + d.getDate());
				...
				if (+d == +today) {
                 ...
                 td.find('div.fc-day-number').removeClass('fc-day-number-header').addClass('fc-day-number-header-today'); //TSS change
				}else{
					td.addClass('fc-not-today')
						.removeClass('fc-today')
						.removeClass(tm + '-state-highlight');
                    td.find('div.fc-day-number').removeClass('fc-day-number-header-today').addClass('fc-day-number-header'); //TSS change
				}
				//--TSS provide hyperlink to day, formatted day number--
                td.find('div.fc-day-number').html("<a href='#' onClick='cal_moveToDay(\"" + d + "\");return false'>" + (((index == 0) || (d.getMonth() != prevMonth)) ? $.fullCalendar.formatDate(d, "MMMM d") : d.getDate()) + "</a>");
                //td.find('div.fc-day-number').html("<a href='#' onClick='cal_moveToDay(\"" + d + "\");return false'>" + d.getDate() + "</a>");
                if (view.name == 'month')//TSS change
                    td.append(generateMoreBoxByDate(d));
                prevMonth = d.getMonth();//TSS change
                index++;//TSS change
                addDays(d, 1);
				if (nwe) {
	...
	function compileSegs(events) {
        ...
        for (i=0; i<rowCnt; i++) {
            ...
            for (j=0; j<row.length; j++) {
                ...
                for (k=0; k<level.length; k++) {
					...
					seg.row = i;
					seg.level = j;
                    if ((seg.level >options.more ) && view.name == 'month'/*TSS change*/)
                    {
                      seg.isHidden = true;
        ...
        {
          var seg = segs[i];
          if (seg.level > options.more && view.name == 'month')
          {
            var dateTime = getOnlyDate(cloneDate(seg.start));
            if (moreSegs[dateTime] == undefined)
            {
              moreSegs[dateTime]= { cnt: 1, row : seg.row };
            }
            else
            {
        ...
        for (var i in moreSegs)
        {
          var seg = new Object();
          var n = moreSegs[i]['cnt'];

          seg.start = new Date(i);
          seg.isStart = true;
          seg.isMore = true;
          seg.isEnd = true;
          seg.msLength = DAY_MS;
          seg.end = addDays(cloneDate(seg.start), 1);
          seg.event = {title : '+ ' + n +' More...', className: ['more']};
          seg.row = moreSegs[i]['row'];
          seg.level = options.more + 1;
          segs.push(seg);
        }
        //var newSegs = segs;
        segs.sort(segCmpMore);
...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
        ...
        if (seg.isMore == undefined)
        {
        html +=
        "<div class='" + className + event.className.join(' ') + ((seg.isHidden)?" seg_"+formatDateForId(seg.start):"")+ "' style='position:absolute;z-index:8;left:"+left+"px;"+((seg.isHidden)?"display:none;":"")+"'>" +
        ...
        {
            html +=
            "<div id='seg_"+formatDateForId(seg.start)+"' class='" + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px;"+((seg.isHidden)?"display:none;":"")+"'>" +
              "<a>" +
                (!event.allDay && seg.isStart ?
                  "<span class='fc-event-time'>" +
                    htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'), options)) +
                ...
                :'') +
                //"<span class='fc-event-title'>" + htmlEscape(event.title) + "</span>" +
                "<span>" + htmlEscape(event.title) + "</span>" +
              "</a>" +
              ((event.editable || event.editable == undefined && options.editable) && !options.disableResizing && $.fn.resizable ?
                "<div class='ui-resizable-handle ui-resizable-" + (rtl ? 'w' : 'e') + "'></div>"
                : '') +
	...
	for (i=0, rowI=0; rowI<rowCnt; rowI++) {
		...
		while (i<segCnt && (seg = segs[i]).row == rowI) {
            if (seg.level != levelI && !seg.isHidden) {
				top += levelHeight;
				levelHeight = 0;
				levelI++;
			}
    ...
    //serg change
    if (view.name == 'month') {//TSS change
        var $moreElements = $('div.more');
        $moreElements.each( function(idx) {
            var seg_id = $(this).attr('id');
            var id = seg_id.replace("seg_", "");
            var uiBoxContent = $('#uibox_content_'+ id);
            uiBoxContent.html('');
            var segs = $("." + seg_id);
            uiBoxContent.append(segs.css({ left: 0, top:0, position: "relative"}).show() );
            segs.find("a").css("height", "auto");//FF, chrome height=100% sets height of parent i.e. 150px
            var $box = $('#uibox_' + id);
            $('#uibox_close_'+ id).bind("click", function(event) {
                $box.find("div.fc-event").css("width", "128px");
                $box.hide();
            });
            $box.css("top", $box.parent().position().top);
            $(this).bind("click", function(event) {
                $box.find("div.fc-event").css("width", "100%");
                $box.show();
            });
        });
...
var viewMethods = {
	...
	sliceSegsByDay: function(events, visEventEnds, start, end) {
		var segs = [],
			i, len=events.length, event,
			eventStart, eventEnd,
			segStart, segEnd,
